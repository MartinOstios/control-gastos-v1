...
def register_user(user: UserCreateSchema = Body()) -> dict:
    try:
        ...
        return JSONResponse(content={"message": "The user was successfully registered","data": jsonable_encoder(new_user)},status_code=status.HTTP_201_CREATED,)
    except Exception as err:
        return JSONResponse(content={"message": str(err), "data": None},status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,)
...
@auth_router.post("/secret",tags=["auth"],response_model=str,description="Test authentication protected route")
